...
userRouter.get("/profile", async (req, res) => {
  ...
  try {
    ...
    const result = await dbPool.query("SELECT username, email, created_at FROM users WHERE id = $1", [userId]);

    if (result.rows.length === 0) {
      return res.send({ success: false, message: "User not found from userRouter" });
    }
